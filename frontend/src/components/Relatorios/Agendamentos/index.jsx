...
const Agendamentos = () => {
    ...
    const handleCloseModal = () => {
        setShowModal(false);
        setSelectedOrder({ order_info: {}, user_info: {} });
    }

    const agendados = data.filter((pedido) => pedido.date_agendada !== null);
    ...
    const currentItems = agendados.slice(indexFirstItem, indexLastItem);

    const totalPages = Math.ceil(agendados.length / itemsPerPage);
    ...
    if (data.length === 0) {
        return <h3 style={{ textAlign: 'center' }}>Nenhum pedido agendado encontrado!</h3>;
    }

    return (
        <>
            <div style={{ maxWidth: '900px', maxHeight: '800px', margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
                    <Bar data={barData} options={barOptions} width={250} height={300} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '10px' }}>
                    <p><strong>Total de Agendamentos:</strong>{' '}<Badge bg="secondary">{agendados.length} ({((agendados.length / data.length) * 100).toFixed(2)}%)</Badge></p>
                </div>
                <div style={{ maxWidth: '80%', margin: '0 auto', overflowX: 'auto' }}>
                    <Table style={{ width: '100%', tableLayout: 'fixed' }}>
                        <thead>
                            <tr>
                                ...
                                <th style={{ width: '20%' }}>Data Agendada</th>
                        ...
                        <tbody>
                            {currentItems.map((pedido) => (
                                <tr key={pedido._id}>
                                    ...
                                    <td>{getFormattedDateAndTime(pedido.date_agendada)}</td>
                    ...
                    <Modal show={showModal} onHide={handleCloseModal}>
                        <Modal.Header closeButton>
                            <Modal.Title>Informações do Pedido {selectedOrder && selectedOrder.order_info._id} (Agendado)</Modal.Title>
                        </Modal.Header>
                                ...
                                selectedOrder && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <p style={{ marginBottom: '10px' }}><strong>Data Agendada:</strong> {getFormattedDateAndTime(selectedOrder.order_info.date_agendada)}</p>
